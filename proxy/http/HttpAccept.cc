...
HttpAccept::mainEvent(int event, void *data)
{
  ...
  if (event == NET_EVENT_ACCEPT) {
   ...
   // copy over session related data.
    ...
    new_session->res_host_res_style = ats_res_calc_style(client_ip->sa_family, host_res_preference);
    new_session->acl_method_mask = acl_method_mask;

    new_session->new_connection(netvc, backdoor);

    Debug("amc", "New connection with host query style %s", HOST_RES_STYLE_STRING[new_session->res_host_res_style]);
